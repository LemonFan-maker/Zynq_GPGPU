; bench_matmul_pp.asm
; Parameterized 8x8 MatMul with Hardware Accumulator Buffer
; A[8][8] at DMEM[base..base+63]
; B[8][8] at DMEM[base+64..base+127]
; Results accumulate directly into acc_buf via MAC_ACC instruction
; acc_ptr is advanced by ACC_NEXT after each output element
...
j_loop:
    ...
    ; === Unrolled k=0..7 MAC_ACC kernel ===
    LDR  r6,  [r4 + 0]     ; A[i][0]
    LDR  r7,  [r5 + 0]     ; B[0][j]
    LDR  r21, [r4 + 1]     ; A[i][1]
    MAC_ACC r6,  r7         ; acc_buf[ptr] += A[i][0]*B[0][j]
    LDR  r22, [r5 + 8]     ; B[1][j]
    LDR  r6,  [r4 + 2]     ; A[i][2]
